...
export class BenchmarkHarness {
	...
	async runTestCase(testCase: TestCase): Promise<TestCaseResult> {
		const workspaceDir = await mkdtemp(
			path.join(tmpdir(), "openclaw-benchmark-"),
		);
		...
		try {
			...
			const results = await this.strategy.search(testCase.query);
	...
	private calculateMetrics(
		...
		return {
			precisionAtK: calculatePrecisionAtKs(
				results,
				testCase.expectedResults,
				kValues,
			),
			recallAtK: calculateRecallAtKs(
				results,
				testCase.expectedResults,
				kValues,
			),
			mrr: calculateMRR(results, testCase.expectedResults),
			totalRelevant: testCase.expectedResults.filter((r) => r.relevance > 0)
				.length,

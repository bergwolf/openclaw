...
export class CurrentStrategy implements Strategy {
  ...
  readonly description = "Production baseline: hybrid search with 70% vector, 30% BM25 text";
  ...
  async initialize(workspaceDir: string): Promise<void> {
    ...
    const memoryConfig = buildMemoryConfig({
      ...
      temporalDecayEnabled: this.config.temporalDecayEnabled as boolean | undefined,
    }) as OpenClawConfig;

    this.manager = await createMemoryManagerOrThrow(memoryConfig, "benchmark-agent");

    await this.manager.sync();
  }

  async search(query: string, _testCaseId?: string): Promise<SearchResult[]> {
    if (!this.manager) {
